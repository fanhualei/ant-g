...
class Database  {
    ...
    /**
     * see the tableDesign.md
     * @param camelColumnName
     * @param columnComment
     * @returns {{extra: (*|string), title: (*|string)}|{extra: string, title: (*|string)}|{extra: string, title: *}}
     * @private
     */
    _getParseComment(camelColumnName,columnComment){
        let ren={};
        const {parseComment} = this.dbConfig;
        if(!parseComment || columnComment.length===0){
            ren ={
                title:camelColumnName,
                extra:''
            }
            return ren;
        }
        const arr = columnComment.split('^');
        if(arr.length===1){
            ren ={
                title:arr[0],
                extra:''
    ...
    async getTablesMap (){
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const characterMaxLength=rows[i].CHARACTER_MAXIMUM_LENGTH;
            ...
            const item = this._getParseComment(camelColumnName,columnComment);
            // console.log(`jsType:${jsType}  dataType:${dataType}  camelTableName=${camelTableName}  ${typeof dataType}`);
    ...
    _getDefaultValue(dataType,columnDefault){
        if(columnDefault === undefined || columnDefault === null || columnDefault.length===0 ){
            ...
            if( 'string' === DbTypeToJc[dataType] ) {
                return `'${columnDefault}'`;
            }else if('number' === DbTypeToJc[dataType]){
