...
module.exports = class extends BasicGenerator {
    ...
    writing() {
        ...
        if(pageConfigData['pageType']==='simple'){
            ...
            this._changeMenu(pageConfigData);
        }
    }

    _changeMenu(pageConfigData){
        const {changeRoute} = this.prompts;
        if(changeRoute) {
            const {getNewMenuMsgCode} = require('../../utils/editjs/localeMsg');
            const filePath = join(this.getProjectPath(), 'src/locales//zh-CN/menu.ts');
            const {title,menu} = pageConfigData;
            const newCode = getNewMenuMsgCode(filePath,`menu.${menu.name}`,'你好');
            this.fs.write(filePath,newCode)
            // fs.writeFileSync(filePath,'你好','utf-8');
        }
    }
    ...
    _changeRoute(pageConfigData){
        const {changeRoute} = this.prompts;
        if(changeRoute){
            const {writeNewRoute} = require('../../utils/edutjs/writeNewRoute');
