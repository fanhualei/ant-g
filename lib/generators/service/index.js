...
class ServiceGenerator extends BasicGenerator {
    ...
    async writing() {
        ...
        const toGTables = serviceConfig["tables"].length>0
            ?serviceConfig["tables"]
            :this.getTableNameArrayByReg(serviceConfig["tableReg"]);
    ...
    _saveOneFile(camelTableName,tableDetail,){
        ...
        this.prettierCode(`src/services/${camelTableName}.d.ts`)
        this.fs.copyTpl(
            this.templatePath("service.d"),
            this.destinationPath(`src/services/${camelTableName}Service.ts`),
            tableDetail
        );
        this.prettierCode(`src/services/${camelTableName}Service.ts`)
    }
